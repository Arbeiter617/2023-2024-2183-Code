...
public class limelightSuckADick extends Command {
    ...
    //      if(idRead == readAprilTags.redsSpeakerTag || idRead == readAprilTags.blueSpeakerTag) {
    //         if(area != 0 && toggleLiemlight.toggleLimelightPos) {
    //             if(prev_distance == 0) {
    //                 prev_distance = area;
    //             } else {
    //                 if(distance + .05 < (prev_distance) && Shooter.shooterSpeed < 1) {
    //                     //System.out.println("Shoot Harder");
        
    //                     Shooter.shooterSpeed = Shooter.shooterSpeed + .1;
    //                 } else if(distance -.05 > (prev_distance) && Shooter.shooterSpeed > 0) {
